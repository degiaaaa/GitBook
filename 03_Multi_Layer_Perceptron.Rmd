...
n_input = len(X[0]) + 1
n_output = len(Y[0])
hidden_layer_neurons = np.array([9,4]) # the 2 means that there is one hidden layer with 2 neurons
...
def backward(IN, OUT, W, Y, grad, k):
  if k == len(grad)-1:
    ...
    grad[k] = ( grad[k+1] @ W[k+1][0:len(W[k+1])-1].T) * deriv_sigmoid(OUT[k]) # ohne das letzte W da wir ja nur die knoten ohen bias befüllen
  return(grad)
