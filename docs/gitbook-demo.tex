...
\[
  Y_0 = step([X_0,_0 \cdot W_0,_0 + X_0,_1 \cdot W_1,_0 + X_0,_2 \cdot W_2,_0])
\]
...
\includegraphics[width=0.5\textwidth,height=\textheight]{./img/NN_01.png}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{def}\NormalTok{ forward(W, x):}
    \ControlFlowTok{return}\NormalTok{( step(W }\OperatorTok{@}\NormalTok{ x) )}
\end{Highlighting}
\end{Shaded}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{X\_train }\OperatorTok{=}\NormalTok{ np.array([}
...
\NormalTok{])}
\NormalTok{W\_0 }\OperatorTok{=}\NormalTok{ np.array([}
