...
hidden_layer_neurons = np.array([11])
...
for i in range(40000):
  ...
  for k in range(len(W)):
    if k==0:
      IN.append(add_ones_to_input(X_train))
    else:
  ...
  for k in range(len(W)-1,-1, -1):
    grad = backward(IN, OUT, W, Y_train, grad, k) 
...
plot_error(mean_square_errors, "MLP Credit Default")
